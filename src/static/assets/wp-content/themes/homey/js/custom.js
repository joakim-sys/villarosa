(function ($) {
  ...
  popover_ele.popover({ trigger: "hover", html: true });
  ...
  if (select_picker.length > 0) {
    select_picker.selectpicker({ dropupAuto: false });
  }
  var isChrome =
  ...
  function WidthChange(mq) {
    if (mq.matches) {
      $(".dashboard-page-title").css({ top: 60 });
      $(".user-dashboard-right").css({ "padding-top": 131 });
    } else {
      $(".dashboard-sidebar").css({
        top: header_area_height + dashboard_header_height + 34,
      });
      $(".user-dashboard-left").css({ top: header_area_height });
      $(".user-dashboard-right").css({
        "padding-top": header_area_height + dashboard_header_height + 4,
      });
      $(".dashboard-page-title").css({ top: header_area_height });
    }
  }
  ...
  function auto_complete() {
    ...
    $(document).mouseup(function (e) {
      ...
      if (
        !input_plus_auto_complete.is(e.target) &&
        input_plus_auto_complete.has(e.target).length === 0
      ) {
  ...
  function homey_period_checkin_checkout(start_date, end_date) {
    ...
    check_in_date.datepicker({ dateFormat: homey_date_format, minDate: today });
    check_in_date.change(function () {
      prev_date = jQuery("#" + start_date).val();
      jQuery("#" + end_date).val("");
...
function add_to_favorite(ajaxurl, listID, curnt, userID, is_singular_listing) {
  if (parseInt(userID, 10) === 0 || userID == undefined) {
    ...
    jQuery.ajax({
      ...
      url: ajaxurl,
      dataType: "json",
      data: { action: "homey_add_to_favorite", listing_id: listID },
      beforeSend: function () {
  ...
  if (listings_compare && listings_compare.length) {
    ...
    if (listings_compare.length) {
      for (var i = 0; i < listings_compare.length; i++) {
        jQuery(
          '.homey_compare[data-listing_id="' + listings_compare[i] + '"]'
        ).text(remove_compare);
      }
