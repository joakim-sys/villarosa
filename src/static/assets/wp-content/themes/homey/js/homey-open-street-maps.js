(function ($) {
  ...
  if (typeof HOMEY_map_vars !== "undefined") {
      ...
      jQuery("input.input-search").autocomplete({
        source: function (request, response) {
          jQuery.get(
            ...
            { format: "json", q: request.term, addressdetails: "1" },
            function (result) {
              if (!result.length) {
    ...
    var homey_osm_marker_position = function (lat, long) {
      ...
      var osmMarkerOptions = { riseOnHover: true, draggable: true };
    ...
    var homey_init_submit_map = function () {
      ...
      homeyMap = L.map("map", { center: mapCenter, zoom: 15 });
      homeyMap.scrollWheelZoom.disable();
    ...
    var homey_submit_autocomplete = function () {
      jQuery("#listing_address").autocomplete({
        ...
        select: function (event, ui) {
          ...
          $("#city, #countyState, #area, #homey_country").selectpicker(
            "refresh"
          );
        },
      });
